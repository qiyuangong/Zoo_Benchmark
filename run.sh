...
case $TYPE in
  ...
  "tf" | "TF")
    CLASS=com.intel.analytics.zoo.example.inference.TFNetPerf
    ;;

  "torch" | "TORCH")
    CLASS=com.intel.analytics.zoo.example.inference.TorchNetPerf
    OPTIONS='-Dbigdl.engineType=mkldnn -Dbigdl.mklNumThreads='${CPU}
    export OMP_NUM_THREADS=${CPU}
    ;;

  "bigdl" | "BIGDL")
    CLASS=com.intel.analytics.zoo.example.inference.BigDLPerf
    OPTIONS='-Dbigdl.engineType=mkldnn -Dbigdl.mklNumThreads='${CPU}
    ;;

  "bigdlblas" | "BIGDLBLAS")
    CLASS=com.intel.analytics.zoo.example.inference.BigDLBLASPerf
    PARM="-c ${CPU}"
    ;;

  "ov" | "OV")
    CLASS=com.intel.analytics.zoo.example.inference.OpenVINOPerf
    export OMP_NUM_THREADS=${CPU}
